...
def generate_training_set_parser():
    descr = "Create a training set for usage in nanite from an .h5 file " \
            + "generated by the nanite-rate command."
    parser = argparse.ArgumentParser(description=descr)
    parser.add_argument('data_path', type=str,
                        help='Path to a rating container or a folder of '
                             + 'rating containers')
    parser.add_argument('out_dir', type=str,
                        help='Path to output training set')
    return parser
